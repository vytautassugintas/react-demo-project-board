...
import {addColumn, addItemToColum, removeColumn, removeItemFromColumn, toggleColumnInput} from '../../../store/actions/board.actions';
...
const boardTarget = {
  drop(props, monitor, component) { 
      ...
      props.dispatch(removeItemFromColumn({columnId: item.columnId, itemId: item.id}));
      props.dispatch(addItemToColum({columnId: props.column.id, text: item.title}));
      return {
         column: props.column
      }
  }
...
class Column extends Component {
  ...
  render() {
    ...
    const items = column.items.map(item => <BoardCard key={item.id} item={item}/>);
...
;
export default connect(mapStateToProps)(DropTarget("ColumnCard", boardTarget, collect)(Column));